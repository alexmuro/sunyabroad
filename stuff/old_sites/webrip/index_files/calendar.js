...
// $Id: calendar.js,v 1.5 2012/09/11 13:10:18 TD.Admin Exp $
...
Calendar.cellClick = function(el, ev) {
	...
	if (typeof el.navtype == "undefined") {
		...
		// added -dain
		// we only rely on the browser's date if todaysDate is not provided -dain
		if (el.navtype == 0) { // navtype of 0 is returned when clicking 'Today'
			if(cal.today != null){
				date = new Date(cal.today); // passed-in date
			} else {
				date = new Date(); // browser date
			}
		} else {
			date = new Date(cal.date); // form provided date
		}
		...
		switch (el.navtype) {
			...
			return;
			case 0:
				newdate = true; // added -dain (we want 'Today' to always close the widget)
...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	// added -dain
	// this.today is now passed in as todaysDate arg. -dain
	if(this.today != null){
...
Calendar.prototype.showAtElement = function (el, opts) {
	...
	function fixPosition(box) {
		...
		//if (Calendar.is_ie) { /* this breaks IE8 -dma */
...
Date.prototype.print = function (str) {
	...
	var re = /%./g;
	// FUSETALK FIXES SAFARI BUG if (!Calendar.is_ie5)
